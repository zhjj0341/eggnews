...
class LinearSelector(FiniteSelector):
  ...
  def select(self,
    ...
    """Returns the index of the next item to be administered.
        ...
        """
    if (index is None or
        self.simulator is None) and (administered_items is None):
      raise ValueError(
          'Either pass an index for the simulator or all of the other optional parameters to use this component independently.'
      )
    ...
    if set(self._indexes) <= set(administered_items):
      warn(
          ...
          .format(self._current, sorted(self._indexes), len(self._indexes),
...
class RandomSelector(Selector):
  ...
  def select(self,
    ...
    """Returns the index of the next item to be administered.
        ...
        """
    if (index is None or
        self.simulator is None) and (items is None or
                                     administered_items is None):
      raise ValueError(
          'Either pass an index for the simulator or all of the other optional parameters to use this component independently.'
      )
...
class ClusterSelector(Selector):
  ...
  def __init__(self,
    ...
    if method not in available_methods:
      raise ValueError(
          '{0} is not a valid cluster selection method; choose one from {1}'.
          format(method, available_methods))
    available_rcontrol = ['passive', 'aggressive']
    if r_control not in available_rcontrol:
      raise ValueError(
          '{0} is not a valid item exposure control method; choose one from {1}'
          .format(r_control, available_rcontrol))
  ...
  @staticmethod
  def sum_cluster_infos(theta: float, items: numpy.ndarray,
                        clusters: list) -> list:
    ...
    for cluster in set(clusters):
      ...
      for item in items[cluster_indexes]:
        cluster_infos[cluster] += irt.inf(theta, item[0], item[1], item[2],
                                          item[3])
  ...
  def avg_cluster_params(items: numpy.ndarray, c: list):
    ...
    occurrences = numpy.bincount(
        numpy.delete(c, numpy.where(c == -1)).astype(numpy.int64))
  ...
  def select(self,
    ...
    try:
      ...
      max_pointer = items.shape[0] if len(
          administered_items) == self._test_size - 1 else slices[
...
class The54321Selector(FiniteSelector):
  ...
  def select(self,
    ...
    organized_items = [
        x for x in irt.inf_hpc(est_theta, items).argsort()
    ...
    organized_items = [
        x for x in numpy.array([
            quad(irt.inf,
                 ...
                 est_theta + self._interval,
                 args=(item[0], item[1], item[2], item[3]))[0] for item in items
